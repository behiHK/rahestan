...
const Layout = ({ children, location }) => {
    ...
    const linkStyle = `
        ...
        hover:shadow-[0_15px_30px_-5px_rgba(0,0,0,0.3)]
   `
    const activeStyle = `
        text-white
        ${isOpened ? "" : "bg-violet-200"}
        cursor-not-allowed
   `
    return (
        <div >
            <div className='flex justify-between items-center px-4 lg:px-20 sm:px-6 h-[10vh]'>
                ...
                <GatsbyLink to={intl.locale === "fa" ? "/en" + locationState : "/" + locationState.split("en/")[1]} className='flex flex-row items-center text-xs text-center hover:text-violet-200 text-gray-900 my-auto mx-2' >
                    {/* <img src={languageIcon} className='w-4 fill' /> */}
                    <p className='text-center'>{intl.locale === "fa" ? "English" : "فارسی"}</p>
                    <svg fill="currentColor" x="0px" y="0px" height="16" width="16" version="1.1" id="XMLID_275_" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className='mx-2 hover:fill-violet-200'>
                        <g id="language">
                            <g>
            ...
            <nav className={`sticky top-0 z-50 px-4 h-[7vh] transition-all duration-150 ease-in text-white bg-gray-600 flex justify-center lg:px-20 sm:px-6 shadow-lg`}>
                ...
                {isOpened &&
                    <section className={`flex flex-col fixed top-0 ${intl.locale === "fa" ? "right-0" : "left-0"} w-4/5 h-full bg-gray-600 lg:hidden z-10`}>
                        <div className={`flex flex-row pt-6 px-6 ${intl.locale === "fa" ? "justify-end" : "justify-start"}`}>
                            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="30" height="30" viewBox="0 0 60 60" fill='#ffffff' onClick={() => setIsOpened(false)}>
                        ...
                        <ul className={`flex flex-col p-6 ${intl.locale === "fa" ? "items-start justify-start  text-right" : "items-end justify-end  text-left"}`}>
                            ...
                            <li className={`text-gray-100 pb-6 ${location.pathname === withPrefix(intl.locale === "fa" ? '/about/' : '/en/about/') ? activeStyle : ""}`}><Link to="/about" onClick={(e) => setIsOpened(false)}> {intl.formatMessage({ id: "aboutUs" })}</Link></li>
                        </ul>
                    </section>
                }

                <div className={`mx-2 w-full flex flex-row  col-start-3 items-center lg:hidden`} onClick={() => setIsOpened(true)}>
                    <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="30" height="30" viewBox="0 0 60 60" fill='#ffffff'>
                ...
                <ul className={`hidden lg:flex ${intl.locale === "fa" ? "flex-row" : "flex-row-reverse"} justify-center items-center h-full ${isScrolled ? "col-start-2  justify-end" : ""} p-0`}>
                    ...
                    <li className={`${linkStyle} ${location.pathname === withPrefix(intl.locale === "fa" ? '/about/' : '/en/about/') ? activeStyle : ""}`}><Link to="/about"> {intl.formatMessage({ id: "aboutUs" })}</Link></li>
                </ul>
            ...
            <footer className={`bg-gray-900 text-gray-100 ${intl.locale === "fa" ? "text-right rtl" : "text-left ltr"}`}>
                <div className={`flex flex-col lg:flex-row justify-between items-center border-solid border-b-2 border-gray-500`}>
                    <div className=' min-h-[30vh] flex flex-col justify-center items-start  py-4 px-10 leading-loose'>
                    ...
                    <img src={logo} className='hidden lg:block h-20 mx-24' />
                </div>
