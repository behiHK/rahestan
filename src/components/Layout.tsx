...
const Layout = ({children, location}) => {
   ...
   `
    return (
        <div >
            <div className='flex justify-end lg:px-20 sm:px-6 h-[12vh]'>
            <Link to="/"><img src={logo} className="cursor-pointer mx-0 my-0 x-auto h-full" /></Link>

            </div>
            <nav className={`sticky top-0 z-50 px-4 h-[8vh] transition-all duration-150 ease-in  ${isScrolled ? "grid gap-0 grid-cols-3 grid-rows-1 grid text-gray-900 bg-white border-solid border-gray-900 border-b-4 lg:px-10 sm:px-4" : "flex justify-center text-white bg-gray-900 lg:px-20 sm:px-6"} shadow-lg` }>
        { isOpened &&
         <section className="flex flex-col fixed top-0 right-0 w-4/5 h-full bg-gray-600 lg:hidden z-10">
         <div className="flex flex-row pt-6 pl-6 justify-start">
           ...
           <ul className="flex flex-col items-end justify-start p-6 text-right">
                ...
                <div className={`mx-2 w-full flex flex-row ${isScrolled ? "justify-start" : "justify-end"} items-center lg:hidden`} onClick={()=> setIsOpened(true)}>
                ...
                <ul className={`hidden lg:flex flex-row-reverse justify-center items-center h-full ${isScrolled ? "col-start-2" : ""} p-0`}>
               ...
               { isScrolled && <div className='flex justify-end col-start-3'><Link to="/"><img src={logo} className="cursor-pointer mx-0 my-0 x-auto h-full" /></Link></div>}
            </nav>

            <main>{children}</main>

            <footer className="text-center bg-gray-900 text-gray-100">
                <div className=' h-[20vh] flex flex-col justify-center items-end text-right border-solid border-b-2 border-gray-500'>
                    ...
                    <p className='pr-10 py-2'>rahestan.co@gmail.com<span className='font-semibold'>&nbsp;&nbsp;:ایمیل</span></p>
                </div>
