...
const ProjectCard = ({img, caption, title, link}) => {
    return (
        <div className={`w-full lg:w-1/3 h-[50vh] bg-no-repeat bg-cover mx-2 my-12 cursor-pointer`} style={{backgroundImage: `url(${img})`}}>
            <div className='w-full h-full bg-red-600 bg-opacity-50 hover:bg-opacity-0 hover:bg-transparent flex flex-col items-start justify-end'>
                ...
                <div className='bg-red-600 hover:bg-red-500 text-white px-6 py-4 mx-10 lg:mx-20 mb-5 rounded-full hover:text-opacity-80 w-48 text-center'><Link to={link} >{title}</Link></div>
            </div>
        </div>
